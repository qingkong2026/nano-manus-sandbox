...
from app.interfaces.schema.shell import (
ShellExecuteReqeust,ShellReadRequest,ShellWaitRequest,ShellWriteRequest,ShellKillRequest
)
from app.interfaces.service_dependencies import get_shell_service
from app.models.shell import ShellReadResult, ShellExecuteResult, ShellWaitResult, ShellWriteResult, ShellKillResult
from app.services.shell import ShellService

router = APIRouter(prefix="/shell", tags=["Shell模块"])


@router.post(path="/exec-command", response_model=Response[ShellReadResult])
async def exec_command(
    request: ShellExecuteReqeust,
    shell_service: ShellService = Depends(get_shell_service),
) -> Response[ShellExecuteResult]:
    """在指定的 Shell 会话中运行命令"""
    # 1.判断是否传递了 session_id，如果不存在则创建一个 session_id
    if not request.session_id or request.session_id == "":
        request.session_id = shell_service.create_session_id()

    # 2.判断是否传递了执行目录，如果未传递则使用根目录作为执行目录
...
async def read_shell_output(
    ...
    """根据传递的 session_id 是否返回控制台标识获取 Shell 命令执行结果"""
    # 1.判断 session_id 对应的 Shell 会话是否存在
    if not request.session_id or request.session_id == "":
        raise BadRequestException("Shell 会话ID 为空，请核实后重试")
...
@router.post(path="/wait-process", response_model=Response[ShellWaitResult])
async def wait_process(
    request: ShellWaitRequest,
    shell_service: ShellService = Depends(get_shell_service)
) -> Response[ShellWaitResult]:
    """传递 session_id 描述执行等待并获取结果"""
    result = await shell_service.wait_process(
        session_id=request.session_id,
...
@router.post(path="/write-shell-input", response_model=Response[ShellWriteResult])
async def write_shell_input(
    request: ShellWriteRequest,
    shell_service: ShellService = Depends(get_shell_service),
) -> Response[ShellWriteResult]:
    """根据传递的会话id+写入内容+按下回车标识向指定子进程写入数据"""
    # 1.判断 session_id 对应的 Shell 会话是否存在
    if not request.session_id or request.session_id == "":
        raise BadRequestException("Shell 会话ID 为空，请核实后重试")
...
@router.post(path="/kill-process", response_model=Response[ShellKillResult])
async def kill_process(
    request: ShellKillRequest,
    shell_service: ShellService = Depends(get_shell_service)
) -> Response[ShellKillResult]:
    """传递 session_id 关闭指定会话"""
    # 1.判断 session_id 对应的 Shell 会话是否存在
    if not request.session_id or request.session_id == "":
        raise BadRequestException("Shell 会话ID 为空，请核实后重试")
