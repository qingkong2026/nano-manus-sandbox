...
from app.interfaces.schema.file import (
    FileReadRequest,
    FileWriteRequest,
    FileReplaceRequest,
    FileSearchRequest,
...
from app.models.file import (
    ...
    FileFindResult,
    FileUploadResult,
    FileDeleteResult, FileCheckResult
)
...
async def read_file(
    request: FileReadRequest, file_service: FileService = Depends(get_file_service)
    ...
    return Response.success(msg="读取文件内容成功", data=result)

@router.post(path="/write-file", response_model=Response[FileWriteResult])
async def write_file(
    request: FileWriteRequest, file_service: FileService  = Depends(get_file_service)
...
async def search_in_file(
    ...
    return Response.success(
        msg=f"文件内搜索完成，找到 {len(result.matches)} 处匹配",
        data=result,
    )
...
async def find_files(
    ...
    return Response.success(
        msg=f"查找完毕,检索到 {len(result.files)}",
        data=result,
    )
...
async def upload_file(
        ...
        filepath: str = Form(None) , # 要上传的文件路径
        file_service: FileService = Depends(get_file_service)
...
@router.post(path="/download-file")
async def download_file(
        filepath: str,
        file_service: FileService = Depends(get_file_service)
) -> FileResponse:
    """根据传递的 filepath 下指的文件"""
    # 1.确保下当前文件是否存在
    await file_service.ensure_file(filepath=filepath)
...
@router.post(path="check-file-exists", response_model=Response[FileCheckResult])
async def check_file_exists(
    ...
    return Response.success(msg="文件存在", data=result)
...
async def delete_file(
    ...
    return Response.success(msg="删除文件成功", data=result)