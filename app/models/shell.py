...
class Shell(BaseModel):
    ...
    output: str = Field(..., description="会话输出结果")
    console_records: List[ConsoleRecord] = Field(
        default_factory=list, description="Shell 会话中控制台记录列表"
    )

    model_config = ConfigDict(
        arbitrary_types_allowed=True,  # 允许 Python 原生对象或自定义的对象作为字段
    )
...
class ShellReadResult(BaseModel):
    ...
    session_id: str = Field(..., description="Shell 会话ID")
    output: str = Field(..., description="Shell 会话输出内容")
    console_records: List[ConsoleRecord] = Field(default_factory=list, description="Shell 会话中控制台记录列表")

class ShellExecuteResult(BaseModel):
    """Shell 命令执行结果"""

    session_id: str = Field(..., description="Shell会话ID")
    command: str = Field(..., description="执行的Shell命令")
    status: str = Field(..., description="命令执行状态")
    returncode: Optional[int] = Field(
        default=None, description="进程返回代码，只有进程结束时才有值"
    )
    output: Optional[str] = Field(
        default=None, description="进程执行结果，只有进程结束时才有值"
    )
