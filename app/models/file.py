...
class FileReadResult(BaseModel):
    ...
    content: str = Field(...,description="要读取的文件内容")

class FileWriteResult(BaseModel):

    filepath: str = Field(...,  description="写入目标文件路径")
...
class FileUploadResult(BaseModel):
    ...
    file_size: int = Field(default=0, description="上传文件的大小，单位字节")
    success: bool = Field(..., description="上传是否成功")

class FileCheckResult(BaseModel):
    """文件检查是否存在结果"""

    filepath: str = Field(..., description="需要检查的文件的绝对路径")
